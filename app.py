...
def get_churn(df_json):
    df = pd.read_json(df_json)
    df["InvoiceDate"] = pd.to_datetime(df["InvoiceDate"])
    # Try progressively smaller churn windows if data range is limited
    total_days = (df["InvoiceDate"].max() - df["InvoiceDate"].min()).days
    for window in [90, 60, 30, max(total_days // 3, 7)]:
        try:
            features = engineer_churn_features(df, churn_window_days=window)
            preds, importance, roc, test, cols = build_churn_models(features)
            return preds, importance, roc, test
        except ValueError:
            continue
    raise ValueError("Not enough data to build churn model")
...
with tab3:
    ...
    try:
        ...
        st.error(f"⚠️ Could not build churn model: {e}")
        st.info("💡 Try selecting a wider date range in the sidebar to provide enough data for the churn model.")
        st.stop()
...
with tab4:
    ...
    if not churn_ok:
        st.error("⚠️ Could not build integrated profiles — churn model failed.")
        st.info("💡 Try selecting a wider date range in the sidebar.")
    else:
