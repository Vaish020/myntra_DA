...
def engineer_churn_features(df: pd.DataFrame, churn_window_days: int = 90):
    ...
    Automatically reduces churn window if data range is too short.
    """
    max_date = df['InvoiceDate'].max()
    min_date = df['InvoiceDate'].min()
    total_days = (max_date - min_date).days
    
    # Adaptively reduce churn window if data range is too short
    if total_days < churn_window_days * 2:
        churn_window_days = max(total_days // 3, 7)  # at least 7 days
    ...
    # Only consider customers whose last purchase was within a reasonable window of cutoff
    # (still "plausibly active" — not ancient one-timers from 2 years ago)
    active_window = min(180, total_days)
    last_purchase = df_features.groupby('Customer ID')['InvoiceDate'].max()
    plausibly_active = last_purchase[last_purchase >= (cutoff_date - pd.Timedelta(days=active_window))].index
